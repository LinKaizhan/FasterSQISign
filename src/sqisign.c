...
//Precomputation for the first ideal to isogeny translation in the signing phase
void precompute_for_first_sign(eichler_package *eich, GEN *x1_gen, GEN *x2_gen, proj *L_kernel, proj* Kd, proj * P3, proj *P, proj *Q, proj *PQ, proj phi_I_basis[], const secret_key *sk){
    ...
    GEN beta_I = lideal_isom(I, I_two); // I*beta = I_two;
    assert(beta_I);    //option 1
    ...
    alpha_last= algmul(A,alpha_last,alg_conj(A,beta_I));
    alpha_last= algmul(A,beta_I,alpha_last);
    alpha_last = algmul(A,alg_scalar(A,gdiv(gen_1,algnorm(A,beta_I,0))),alpha_last);
    ...
    GEN L_i = last_I;
    two_walk phi_dual;
    const two_walk_long *phi_I = &sk->phi_two;
    ...
    long f = two_tors_height;
    assert(lideal_isom(I,I_two));

    bool special_fixed =  true;

    //this  is the main function of the loop
    assert(alglatcontains(A,lideal_right_order(I),lideal_generator(L_i),NULL));

    *L_kernel = phi_I_basis[6];
    A = lideal_algebra(I);
    GEN twof = gpowgs(gen_2,f);

    order = lideal_order(I);

    GEN fm = famat_sqr(famat_mul(global_setup.gen_p_plus_fact, global_setup.gen_p_minus_fact));
    GEN gamma = gen_0, beta;
    GEN trace = gen_2;
    
        if (special_fixed) {
            eichler_norm_equation_special_smooth_small_curve_fixed(eich,I, L_i, fm, gen_2);
        }
        beta = eich->beta;
        trace = gmod(gmul(gen_2,gel(beta,1)),twof);
        if (eich->delta) {
    ...
    if(uintbig_equal(&x1,&uintbig_1)||uintbig_equal(&y1,&uintbig_1))
    {
        xBIDIM_one(&ker1_plus, &endpoint, &phi_I_basis[0], &x1, &phi_I_basis[1], &y1, &phi_I_basis[2]);
    }
    else
    {
        xBIDIM(&ker1_plus, &endpoint, &phi_I_basis[0], &x1, &phi_I_basis[1], &y1, &phi_I_basis[2]);
    }
    gentobig(&x1, gel(v1_minus,1));
    gentobig(&y1, gel(v1_minus,2));
    if(uintbig_equal(&x1,&uintbig_1)||uintbig_equal(&y1,&uintbig_1))
    {
        xBIDIM_one(&ker1_minus, &endpoint, &phi_I_basis[3], &x1, &phi_I_basis[4], &y1, &phi_I_basis[5]);
    }
    else
    {
        xBIDIM(&ker1_minus, &endpoint, &phi_I_basis[3], &x1, &phi_I_basis[4], &y1, &phi_I_basis[5]);
    }
    ...
    if(uintbig_equal(&x2,&uintbig_1)||uintbig_equal(&y2,&uintbig_1))
    {
        xBIDIM_one(&ker2_plus, &endpoint, &phi_I_basis[0], &x2, &phi_I_basis[1], &y2, &phi_I_basis[2]);
    }
    else
    {
        xBIDIM(&ker2_plus, &endpoint, &phi_I_basis[0], &x2, &phi_I_basis[1], &y2, &phi_I_basis[2]);
    }
    gentobig(&x2, gel(v2_minus,1));
    gentobig(&y2, gel(v2_minus,2));
    if(uintbig_equal(&x2,&uintbig_1)||uintbig_equal(&y2,&uintbig_1))
    {
        xBIDIM_one(&ker2_minus, &endpoint, &phi_I_basis[3], &x2, &phi_I_basis[4], &y2, &phi_I_basis[5]);
    }
    else
    {
        xBIDIM(&ker2_minus, &endpoint, &phi_I_basis[3], &x2, &phi_I_basis[4], &y2, &phi_I_basis[5]);
    }
    ...
    deterministic_second_point(Kd,&phi_dual.A);
    proj P4;
    xBILIFT(P3,&P4,L_kernel,Kd,&phi_dual.A);
    ...
    if(f == two_tors_height){
        ...
        fp2_add2(&WM.z, &WA.z);
        fp2 t1, t2, t3, t4;
        fp2_mul3(&t1, &WM.z, &xyP1.z);
        fp2_mul3(&t2, &t1, &xyP2.z);
        fp2_mul3(&t3, &t2, &xyQ.z);
        fp2_inv(&t3);
        fp2_mul3(&t4, &t3, &xyQ.z);
        fp2_mul3(&xyQ.z, &t3, &t2);
        fp2_mul3(&t3, &t4, &xyP2.z);
        fp2_mul3(&xyP2.z, &t4, &t1);
        t1 = xyP1.z;
        fp2_mul3(&xyP1.z, &t3, &WM.z);
        fp2_mul3(&WM.z, &t3, &t1);
        fp2_mul2(&WM.x, &WM.z);
        WM.z = fp2_1;
        fp2_sq2(&WA.x, &WM.x);
        fp2_add3(&WA.z, &WA.x, &WA.x);
        fp2_add2(&WA.x, &WA.z);
        fp2_sub3(&WA.x, &fp2_1, &WA.x);
        WA.z = WM.z;
        fp2_mul3(&(xyP1.x),&(xyP1.x),&(xyP1.z));
        fp2_mul3(&(xyP1.y),&(xyP1.y),&(xyP1.z));
        xyP1.z = fp2_1;
        fp2_mul3(&(xyP2.x),&(xyP2.x),&(xyP2.z));
        fp2_mul3(&(xyP2.y),&(xyP2.y),&(xyP2.z));
        xyP2.z = fp2_1;
        fp2_mul3(&(xyQ.x),&(xyQ.x),&(xyQ.z));
        fp2_mul3(&(xyQ.y),&(xyQ.y),&(xyQ.z));
        xyQ.z = fp2_1;
        fp2_add2(&(xyP1.x), &WM.x);
        fp2_add2(&(xyP2.x), &WM.x);
        fp2_add2(&(xyQ.x), &WM.x);
        
        fp2 val1 = fp2_1, val2 = fp2_1, val3 = fp2_1;
        pairing_share(&val1, &val2, &val3, &xyP1, &xyP2, &xyQ, &WA, f);
        int column_length = two_tors_height/win;
        int row_length = pow(2,win-1);
        int ans[column_length];
        PH_DLP(&x2, ans, val2, Strategy, win, column_length, row_length,f);
        *x2_gen = bigtogen(&x2);
        *x2_gen = gsub(twof, *x2_gen);
        PH_DLP(&x1, ans, val3, Strategy, win, column_length, row_length,f);    
        *x1_gen = bigtogen(&x1);
        uintbig x0;
        PH_DLP(&x0, ans, val1, Strategy, win, column_length, row_length,f);
        GEN x0_gen = bigtogen(&x0);
        x0_gen = gmod(gdiv(gen_1, x0_gen), twof);
        *x1_gen = gmod(gmul(*x1_gen,x0_gen), twof);
        *x2_gen = gmod(gmul(*x2_gen,x0_gen), twof);
    }
    else{
        proj2 xyP12;
        ...
        *x2_gen = gmod(gmul(*x2_gen,H2_norm),twof);
        ...
        //use the trace and the determinant to verify the signs
        GEN H1_norm = gmod(lideal_norm(H1),twof);
        GEN N_b = gmod(gmul(H1_norm, H2_norm), twof);
        x4_gen = gmod(gsub(trace,*x1_gen),twof);
        x3_gen = gmod(gdiv(gsub(gmul(*x1_gen,x4_gen),N_b),*x2_gen),twof);
        GEN H2_norm_inv = gmod(gdiv(gen_1, H2_norm), twof);
        x3_gen = gmod(gmul(x3_gen, H2_norm_inv), twof); 
        x4_gen = gmod(gmul(x4_gen, H2_norm_inv), twof);
        
        proj tmp_point;
        gentobig(&x3,x3_gen);
        gentobig(&x4,x4_gen);
        xBIDIM(&tmp_point, &A2, &pt[0], &x3, &pt[1], &x4, &pt[2]);
        if(!mont_equal(&tmp_point,&P1[1]))
        {
            *x1_gen = gmod(gneg(*x1_gen),twof);
            *x2_gen = gmod(gneg(*x2_gen),twof);
        }
  }
  find_basis_2e(P,Q,PQ,&phi_dual.A);
}
...
long normalized_walk(two_walk *w,uintbig *zip, long *n) {
  ...
  if (add_step == 1){
    ...
    xBIDIM_one(&phi_test.ker,&phi_test.A,&P1,&zip[step],&P2,&uintbig_1,&P3);
    isomorphism isom2;
  ...
  *n=step+add_step;
  ...
  for (int i = 0; i < step; i++) {
    ...
    xBIDIM_one(&(walk[i].ker), &A, &P, &zip[i], &Q, &uintbig_1, &PQ);
...
void challenge(proj *E_cha, two_walk_long* phi_chall_even, const uintbig *m, const proj *E_com, const proj *basis_plus, const proj *basis_minus, GEN *dlog, proj *basis_two){
    ...
    if (need_even_chall) {
      ...
      uintbig one;
      uintbig_set(&one,1);
      xBIDIM_one(&isog_chall.ker,&A,&P2,&one,&P1,&even_chall,&P12);
      #ifndef NDEBUG
...
void decompress(two_walk *walk, proj *A, const uintbig *zip, long len,long last_step) {
  ...
  xBIDIM_one(&walk[len-1].ker, A, &P, &zip[len-1], &Q, &uintbig_1, &PQ);
  walk[len-1].A = *A;
...
void decompress_new(two_walk *walk,proj *A, const uintbig *zip, long len,long last_step) {
  ...
  normalize_proj(A);
  find_basis_2e(&P, &Q, &PQ, A);
  xBIDIM_one(&(walk[0].ker), A, &P, &zip[0], &Q, &uintbig_1, &PQ);
  walk[0].A = *A;
  walk[0].len = two_tors_height;
  eval_walk(&walk[0],A,&P);
  //next steps
  for (int i =1 ; i< len-1; i++) {
    deterministic_second_point(&Q,A);
      ...
      //it is set to delta when it allows us to do one less step
       if (len_tail != 0) {
         ...
         if (len_tail == delta) {
           ...
           deterministic_second_point(&R,&A_fin);
           for (int i =0 ; i<two_tors_height -  len_tail; i++) {
             xDBL(&R,&A_fin,&R);
           }
...
// response with precomputation
void response_new2(two_walk_long *sigma,uintbig *zip, GEN coeff_ker_challenge_commitment, GEN I_even , const secret_key *sk, const proj *basis_two, const proj *E_cha, bool *test, GEN *x1_gen, GEN *x2_gen, proj *L_kernel, proj *Kd, proj *P3, proj *P, proj *Q, proj *PQ, proj *phi_I_basis, eichler_package *eichler){
    pari_sp ltop = avma;
    GEN A = global_setup.B;
    GEN order = global_setup.O0;
    ...
    *test = false;
    //endpoint will indicate the endpoint of the translated isogeny
    proj endpoint,A_fin;
    ideal_to_isogeny_two_new2(sigma,sk->I_two,sk->I_large_prime,L_cut,last_I,&sk->phi_two,zip,test,&endpoint, x1_gen, x2_gen, L_kernel, Kd, P3, P, Q, PQ, phi_I_basis, eichler);
    A_fin = sigma->phi[sigma->len-1].A;
    // deterministic_second_point(&Q,&sigma->phi[sigma->len-1].ker,&sigma->phi[sigma->len-1].A,two_tors_height);
    eval_walk(&sigma->phi[sigma->len-1],&A_fin,&endpoint);
    if (*test) {
      ...
      //it is set to delta when it allows us to do one less step
       if (len_tail != 0) {
         ...
         if (len_tail == delta) {
           ...
           xBILIFT(&QR,&temp,&QQ,&R,&A_fin);
           points[0]= R;points[1]=QQ;points[2]=QR;
           two_walk eta;
           //
           bool done;
           proj from = A_fin;
...
// The first ideal to isogeny translation of the signing phase with precomputation
void first_sign(two_walk *phi, uintbig *res, GEN *x1_gen, GEN *x2_gen, proj *L_kernel, proj *Kd, proj *P3, proj *P, proj *Q, proj *PQ, eichler_package *eich, GEN I, GEN I_two,
   GEN K, GEN L , proj* phi_I_basis, proj *base_curve, long f, bool special_fixed){
    GEN twof = gpowgs(gen_2,f);
    ...
    *x1_gen = gmod(gadd(*x1_gen,gel(coeffs,1)) ,twof);
    ...
    if (special_fixed) {
        ...
        *res = res_temp;
        xBIDIM_one(&(phi->ker), &phi->A, P, &res_temp, Q, &uintbig_1, PQ);
      
        #ifndef NDEBUG
        proj P_test;
        xBIDIM_one(&(P_test), &phi->A, P, &res_temp, Q, &uintbig_1, PQ);
        Pt1 = P_test;
        for (int i=1;i<f;i++) {
            xDBL(&Pt1,&phi->A,&Pt1);
        }
...
void sign(compressed_signature *comp_sigma, const secret_key *sk, const public_key *pk, const uintbig *m) {
    ...
    eval_mult_3pt(&comp_sigma->E_com, &phi_com, points, 9);
...
void sign_new(compressed_signature *comp_sigma, const secret_key *sk, const public_key *pk, const uintbig *m) {
    ...
    if (len_tail == 0 && !need_even_chall){
      eval_mult_3pt(&comp_sigma->E_com, &phi_com, points, 6);
    }
    else {
      eval_mult_3pt(&comp_sigma->E_com, &phi_com, points, 9);
    }
...
// sign with precomputation
void sign_new2(compressed_signature *comp_sigma, const secret_key *sk, const public_key *pk, const uintbig *m, GEN *x1_gen, GEN *x2_gen, proj *L_kernel, proj *Kd, proj *P3, proj *P, proj *Q, proj *PQ, proj *phi_I_basis, eichler_package *eichler) {
    pari_sp ltop = avma;
    GEN coeff_com, I_com,I_even;
    two_walk_long phi_com_even,phi_chall_even;
    init_trivial_two_walk_long(&phi_com_even);
    ...
    init_trivial_two_walk_long(&sigma);
    bool test;
    response_new2(&sigma, comp_sigma->zip, coeff_ker_challenge_commitment, I_even, sk, basis_two, &E_cha,&test, x1_gen, x2_gen, L_kernel, Kd, P3, P, Q, PQ, phi_I_basis, eichler);
