...
bool MITM_cutoff(two_walk *phi, const proj *from, const proj *to, long len, long tab_size) {
  ...
  for (uint64_t i = 0; i < 1 << tab_size; i++) {
    uintbig_set(&a, i);
    xBIDIM_one(phi_kers, from, &Pf, &a, &Qf, &uintbig_1, &PQf);
    B = *from;
  ...
  for (uint64_t i = 0; i < 1 << dual_len; i++) {
    uintbig_set(&a, i);
    xBIDIM_one(dual_kers, to, &Pt, &a, &Qt, &uintbig_1, &PQt);
    B = *to;
      ...
      if (start >= 0) {
        ...
        *i1 = h_tab[start].a;
        ...
        if (fp2_equal(&j1_fp[*i1], &j2_fp[i2])) {
          ...
          uintbig_set(&a, i2);
          xBIDIM(&phi2.ker, B, Q3, &a, Q1, &uintbig_1, &Q13);
...
bool MITM2(two_walk *eta, const proj *from, const proj *to, long length) {
  ...
  for (int head = 0; head < (1 << head_len); ++head) {
    uintbig_set(&a, head);
    xBIDIM_one(&head_walk.ker, to, &Q3, &a, &Q1, &uintbig_1, &Q13);
    K = Q3_long;
    ...
    if (found) {
      eta->ker = K;
      uintbig_set(&a, i1);
      xBIDIM_one(&phi1.ker, from, &P3, &a, &P1, &uintbig_1, &P13);
