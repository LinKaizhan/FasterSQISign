...
void eval_rtl_rec_3pt(proj *A, proj *K, int len, bool advance,proj *P, int stacklen, long fact) {

  if (len==0) return;

  if (len==1) {
    xISOG_many_3pt(A, P, stacklen, K, fact);
  } else {
    long right = len / 2;
    long left = len - right;
    ...
    eval_rtl_rec_3pt(A, K, right,advance, P, stacklen+1,fact);
    K[right*advance]=P[stacklen];

    eval_rtl_rec_3pt(A,K+right*advance,left,advance,P,stacklen,fact);
  }
}
...
//Internal function
//supersedes eval_rtl_old using the recursive function for better isogeny computation
void eval_rtl_3pt(proj *A, proj *Q, int n, isog_degree deg, const long* fact, long len) {
  uintbig cof;
  degree_to_uint(&cof, deg, fact, len);
  ...
  int start=0;
    ...
    if (fact[0]<6 && v>8) {
      ...
      for (int i=0; i<n;i++)
        stack[i]=*(Q+i);
      eval_rtl_rec_3pt(A, &K, v, false, stack, n, fact[0]);
      // assert(mont_iszero(&stack[1]));
      for (int i=0;i<n;i++)
          *(Q+i)=(stack[i]);
  ...
  for (int i = len - 1; i >= start; i--) {
    ...
    if (fact[i] < 6 && v != 0) {
      ...
      for (int i=0; i<n;i++)
        stack[i]=*(Q+i);
      eval_rtl_rec_3pt(A, &K, v, false, stack, n, fact[i]);
      assert(mont_iszero(&stack[1]));
      for (int i=0;i<n;i++)
          *(Q+i)=(stack[i]);
    }
...
void eval_mult_3pt(proj *A, const odd_isogeny *phi, proj *P, int n) {
  proj Q[n+2];
  Q[0] = P[0];
  ...
  eval_rtl_3pt(A, Q, n+2, phi->deg_plus, p_plus_fact, p_plus_len);

  // The p-1 part
  Q[1] = Q[n+1];
  eval_rtl_3pt(A, Q, n+1, phi->deg_minus, p_minus_fact, p_minus_len);
