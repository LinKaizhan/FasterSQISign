...
GEN klpt_strong_approximation(GEN A, GEN order, GEN p, GEN N, GEN beta, GEN gamma, GEN L, GEN lambda) {
    ...
    for (int i = 0; i < lg(gel(small_vectors,3)); ++i) {
        ...
        if(gequal(gmod(gel(diff,1), gen_2), gen_1) || gequal(gmod(gel(diff,2),gen_2), gen_1)){
        // if(!gequal(gmod(gel(diff,1), gen_2), gmod(gel(diff,2),gen_2))){
            ...
            if ((q != 1 && ispseudoprime(rhs,0) && cornacchia(stoi(q), rhs, &ap, &bp))
                ...
                GEN betap;
                 if (gequal( gmod(a,gen_2),gmod(d,gen_2))) {
                    betap = mkcol4(gdiv(a,gen_2),gdiv(b,gen_2),gdiv(c,gen_2),gdiv(d,gen_2));
                  }
                  else {
                    GEN a_temp = a;
                    a = b;
...
GEN klpt_general_power_given_J_fixed_norm(GEN I, GEN l, GEN J, GEN delta) {
    ...
    do {
      ...
      GEN n2;
      gamma = alg_primitive(&n2, A, order, gamma);
      GEN n1=n2;
      GEN A_1pi = mkcol4s(1,1,0,0);
      GEN tmpg = algmul(A,A_1pi,gamma);
      tmpg = alg_primitive(&n2, A, order, tmpg);
      if(gequal(n2, gen_1))
      {
      ...
      {
        gamma = tmpg; 
        L2_exp = signing_length - L1_exp + 3 + 2*Z_lval(ggcd(n1,L1),2);
      } 
      tmpg = algmul(A,gamma,A_1pi);
      tmpg = alg_primitive(&n2, A, order, tmpg);
      if(!gequal(n2,gen_1))
      {
        gamma = tmpg;
    ...
    if (mu) {
        ...
        GEN generatorJ = lideal_generator_coprime(J,l);
        GEN generator = algmul(A,generatorJ,alg_conj(A,alpha));
        GEN norm = powis(l, signing_length);
        GEN newideal = lideal_create(A, order, generator, norm);
        return gerepilecopy(ltop, newideal);
    }
    return NULL;
