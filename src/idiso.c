...
void deterministic_second_point(proj *Q,proj* A) {
  ...
  while (true) {
    ...
    for (int i = 1; i < two_tors_height; i++)
      xDBL(&Q2, A, &Q2);
    if (!mont_iszero(&Q2) && !fp2_equal(&Q2.x, &fp2_0))
      break;
  }
...
void ideal_to_isogeny_small_two_f(two_walk *phi, uintbig *res, eichler_package *eich ,  GEN I, GEN I_two,
  ...
  GEN gamma, beta, gamma_wit;
    ...
    {
        xBIDIM_one(&ker1_plus, base_curve, &phi_I_basis[0], &x1, &phi_I_basis[1], &y1, &phi_I_basis[2]);
    }
    else
    {
    ...
    if(uintbig_equal(&x1,&uintbig_1)||uintbig_equal(&y1,&uintbig_1))
    {
        xBIDIM_one(&ker1_minus, base_curve, &phi_I_basis[3], &x1, &phi_I_basis[4], &y1, &phi_I_basis[5]);
    }
    else
    {
    ...
    {
        xBIDIM_one(&ker2_plus, base_curve, &phi_I_basis[0], &x2, &phi_I_basis[1], &y2, &phi_I_basis[2]);
    }
    else
    {
    ...
    if(uintbig_equal(&x2,&uintbig_1)||uintbig_equal(&y2,&uintbig_1))
    {
        xBIDIM_one(&ker2_minus, base_curve, &phi_I_basis[3], &x2, &phi_I_basis[4], &y2, &phi_I_basis[5]);
    }
    else
    {
    ...
    proj Kd;
    deterministic_second_point(&Kd,base_curve);
    ...
    GEN x1_gen, x2_gen, x3_gen, x4_gen;
    //compute the DLPs
    uintbig x3,x4;
    proj2 xyP1,xyP2,xyQ;
    xtoxy(&xyP1,&A2,&pt[0]);
    xtoxy(&xyP2,&A2,&pt[1]);
    xtoxy(&xyQ,&A2,&P1[0]);
    fp2 t1, t2, t3, t4, t5, t6;
    fp2_mul3(&t1, &pt[2].x, &A2.z);     //Az*x3
    fp2_mul3(&t2, &A2.x, &pt[2].z);     //Ax*z3
    fp2_add2(&t1, &t2);                 //Az*x3+Ax*z3
    fp2_mul3(&t2, &A2.z, &pt[2].z);     //z3*Az
    fp2_mul3(&t3, &pt[0].x, &pt[1].z);  //x1*z2
    fp2_mul3(&t4, &pt[1].x, &pt[0].z);  //z1*x2
    fp2_add3(&t5, &t3, &t4);            //x1*z2+z1*x2
    fp2_mul3(&t6, &pt[0].z, &pt[1].z);  //z1*z2
    fp2_mul2(&t1, &t6);
    fp2_mul2(&t5, &t2);
    fp2_add2(&t1, &t5);
    fp2_mul2(&t2, &t6);
    ...
    fp2_sq1(&t3);
    fp2_mul2(&t1, &t3);
    fp2_mul3(&t4, &xyP1.y, &xyP2.z);
    fp2_mul3(&t5, &xyP1.z, &xyP2.y);
    fp2_sub2(&t4, &t5);
    fp2_sq1(&t4);
    fp2_mul2(&t2, &t4);
    if(!fp2_equal(&t1, &t2))
    ...
    if(f == two_tors_height){
        ...
        //use the reduced trace and the reduced norm to verify the signs
    ...
    if (special_fixed) {
      ...
      *res = res_temp;
      xBIDIM_one(&phi->ker,base_curve,&L_kernel,&res_temp,&Kd,&uintbig_1,&P3);
      #ifndef NDEBUG
...
void ideal_to_isogeny_two_new2(two_walk_long *phi_res,
    GEN I,GEN J, GEN K, GEN L, const two_walk_long *phi_I, uintbig *zip,
    bool *endpoint_close_to_E0,proj *endpoint, GEN *x1_gen, GEN *x2_gen, proj *L_kernel, proj *Kd, proj *P3, proj *P, proj *Q, proj* PQ, proj *phi_I_basis, eichler_package* eichler){
    pari_sp ltop = avma;
    GEN A = lideal_algebra(J);
    ...
    *endpoint = phi_dual.A;
    ...
    for (int i = 0; i < steps-1; ++i) {
        ...
        if(i == 0)
        {
            first_sign(&phi[0], &zip[0], x1_gen, x2_gen, L_kernel, Kd, P3, P, Q, PQ, eichler, I_i, I_two_i, K_i_short, L_i, phi_I_basis, &(phi_dual.A),f, special_fixed);
            eich.beta = eichler->beta; eich.J = eichler->J; eich.delta = eichler->delta; eich.I = eichler->I; eich.L = eichler->L;
            eich.gamma = eichler->gamma;
        }
        else
        {
            ideal_to_isogeny_small_two_f(&phi[i],&zip[i],&eich,I_i,I_two_i,K_i_short,L_i,phi_I_basis,&(phi_dual.A),f,special_fixed,other_special_extremal);
        ...
        if (eich.delta) {
          ...
          assert(mont_iszero(&P_test3));
          #endif
          //push the points through the new isogeny
          eval_walk_mult(&phi[i], endpoint, phi_I_basis, 7);
          phi_dual.A = *endpoint;
          phi_dual.ker=phi_I_basis[6];
          phi_dual.len = phi[i].len;
...
void ideal_to_isogeny_1728(two_walk_long* phi ,GEN I,proj *pk) {
  ...
  proj Poi;
  deterministic_second_point(&Poi,&phi_I_target);
  proj poin2 = Poi;
  for (int i =1;i< len_step;i++) {
  ...
  xytox(&p1, &Pt1);
  xytox(&p2, &Pt2);
  xBILIFT(&p3,&p4,&p1,&p2,&phi_I_target);
  xBIDIM(&P_ker1, &phi_I_target, &p1, &C, &p2, &D, &p3);
  isomorphism isom;
